...
class CheckoutController extends Controller
{
    ...
    public function store_shipping_info(Request $request)
    {
        if (Auth::check()) {
            ...
            $data['checkout_type'] = $request->payment_option;
            ...
            if(isset($request->Shiping_different_address) && $request->Shiping_different_address == 1){
                ...
                $ship['firstname'] = $address->first_name;
                $ship['lastname'] = $address->last_name;
                $ship['address'] = $address->address;
                $ship['email'] = $address->email;
                $ship['country'] = $address->country;
                $ship['state'] = $address->state;
                $ship['city'] = $address->city;
                $ship['postal_code'] = $address->postal_code;
                $ship['phone'] = $address->phone;
                $ship['checkout_type'] = $request->payment_option;
                $ship['order_notes'] = $request->order_notes;
                $ship['type'] =1;
            }
        }else{
            $validated = $request->validate([
            ...
            if(isset($request->Shiping_different_address) && $request->Shiping_different_address == 1){ 
                ...
                'ship_country_region' => 'required',
                'ship_street_address' => 'required',
                'ship_postcode_zip' => 'required',
                'ship_phone' => 'required',
                'ship_province_state' => 'required',
                'ship_email_address' => 'required',
            ]);
        }
            ...
            $data['address'] = $request->street_address;
            $data['country'] = $request->country_region;
            $data['state'] = $request->province_state;
            $data['city'] = $request->town_city;
            $data['postal_code'] = $request->postcode_zip;
            $data['phone'] = $request->phone;
            $data['checkout_type'] = $request->payment_option;
            $data['order_notes'] = $request->order_notes;
            $data['type'] = 0;
            if(isset($request->Shiping_different_address) && $request->Shiping_different_address == 1){
                $ship['firstname'] = $request->ship_first_name;
                $ship['lastname'] = $request->ship_last_name;
                $ship['email'] = $request->ship_email_address;
                $ship['address'] = $request->ship_street_address;
                $ship['country'] = $request->ship_country_region;
                $ship['state'] = $request->ship_province_state;
                $ship['city'] = $request->ship_town_city;
                $ship['postal_code'] = $request->ship_postcode_zip;
    ...
    public function remove_coupon_code(Request $request){
        $request->session()->forget('coupon_id');
        $request->session()->forget('coupon_discount');
        flash('Coupan removed successfully!')->success();
        return back();
    }
