...
if (! function_exists('TotaldiscountPercent')) {
    function TotaldiscountPercent($id)
    {
        $product = Product::findOrFail($id);
        $percent = "";
        if ($product->discount_type == 'percent') {
            $percent = $product->discount;
                ...
                $amount = $product->unit_price - $product->discount;
            $percent = ((($product->unit_price -  $amount ) *100) / $product->unit_price);
        }
